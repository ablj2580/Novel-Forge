import { initState, getState, getApiConfig, setApiConfig, persist, persistApiConfig, activeBookId, getModules, getModuleName, addActivity } from './state.js';
...
import { showToast, addLog, clearLogs, switchView, render, getSelectedNoteId, setSelectedNoteId, setSelectedModuleId, setSelectedChapterId, setReferenceModuleFilters, getReferenceModuleFilters, toggleReferenceFilter, setLogFilter, showModuleDialog, currentChapter, updateSearchUI, renderSearchSuggestions, hideSearchSuggestions, switchToFocusMode, exitFocusMode, toggleFocusChaptersPanel, toggleFocusReferencesPanel, renderFocusMode, renderFocusChapters, renderFocusEditor, renderFocusReferences } from './render.js';
import { renderRelationGraph, setSelectedRelationItem, addRelation } from './relation.js';
...
const showCropDialog = (imageSrc, bookId) => {
  ...
  const updateImagePosition = () => {
    cropImage.style.transform = `scale(${cropZoom.value}) translate(${offsetX}px, ${offsetY}px)`;
  };
  ...
  const handleApply = () => {
    ...
    const frameLeft = (frameWidth - imgScaledWidth) / 2 + offsetX;
    const frameTop = (frameHeight - imgScaledHeight) / 2 + offsetY;
    
    const sourceX = (-frameLeft) * scale;
    const sourceY = (-frameTop) * scale;
    const sourceWidth = frameWidth * scale;
    const sourceHeight = frameHeight * scale;
    
    ctx.drawImage(
      img,
      Math.max(0, Math.min(sourceX, img.naturalWidth - sourceWidth)),
      Math.max(0, Math.min(sourceY, img.naturalHeight - sourceHeight)),
  ...
  cropImage.onload = () => {
    const scale = Math.max(cropFrame.offsetWidth / cropImage.naturalWidth, cropFrame.offsetHeight / cropImage.naturalHeight) * 1.5;
...
export const updateFocusChapter = () => {
  ...
  chapter.body = dom.focusChapterBodyInput.value;
  chapter.updatedAt = nowIso();
