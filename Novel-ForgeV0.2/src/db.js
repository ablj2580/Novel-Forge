...
export const initDB = () => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!database.objectStoreNames.contains('favoriteRelations')) {
        const favStore = database.createObjectStore('favoriteRelations', { keyPath: 'id' });
        favStore.createIndex('bookId', 'bookId', { unique: false });
        favStore.createIndex('itemId', 'itemId', { unique: false });
      }
    };
  });
};
...
export const saveFavoriteRelations = (favoriteRelations) => {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(['favoriteRelations'], 'readwrite');
    const store = transaction.objectStore('favoriteRelations');
    
    favoriteRelations.forEach(fav => {
      store.put(fav);
    });
...
export const loadFavoriteRelations = () => {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(['favoriteRelations'], 'readonly');
    const store = transaction.objectStore('favoriteRelations');
    const request = store.getAll();
