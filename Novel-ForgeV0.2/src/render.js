...
import { getState, getApiConfig, activeBook, activeBookId, getModules, getModuleName, addActivity, getFavoriteLibraryItems, isReferenceHighlighted, getHighlightedReferences } from './state.js';
import { wordCount, escapeHtml, formatDate, highlightSearchTerm, renderRichText } from './utils.js';
...
export const getShowFavoritesOnly = () => showFavoritesOnly;
...
export const setShowFavoritesOnly = (value) => { showFavoritesOnly = value; };
export const toggleFavoritesOnly = () => { showFavoritesOnly = !showFavoritesOnly; };
...
export const renderNoteDetail = () => {
  ...
  const suggestion = note.suggestion;
  const tagsInputValue = suggestion?.tags?.join(", ") || "";
  
  const books = state.books;
  const bookOptions = books.map(b => `
    <option value="${b.id}" ${suggestion?.bookId === b.id ? 'selected' : ''}>${escapeHtml(b.title)}</option>
  `).join('');
  
  const modules = getModules();
  const moduleOptions = modules.map(m => `
    <option value="${m.id}" ${suggestion?.module === m.id ? 'selected' : ''}>${m.name}</option>
  `).join('');
    ...
    <article>
      <div class="note-edit-section">
        ...
        ${
          suggestion
            ? `
              <div class="note-book-edit">
                <label class="book-label">存入作品</label>
                <select id="noteBookSelect" class="note-book-select" disabled>
                  ${bookOptions}
                </select>
              </div>
              <div class="note-module-edit">
                <label class="module-label">目标模块</label>
                <select id="noteModuleSelect" class="note-module-select" disabled>
                  ${moduleOptions}
                </select>
              </div>
              <div class="note-tags-edit">
                <label class="tags-label">标签</label>
                <input 
                  type="text" 
                  id="noteTagsInput" 
                  class="note-tags-input" 
                  value="${escapeHtml(tagsInputValue)}" 
                  readonly
      ...
      ${
        suggestion
          ? `
            <div class="tag-row" id="noteTagsDisplay">
              <span class="module-pill">${getModuleName(suggestion.module)}</span>
              ${(suggestion.tags || []).map((tag) => `<span class="tag">${escapeHtml(tag)}</span>`).join("")}
            </div>
      ...
      <div class="card-actions">
        <button class="secondary-button" data-action="edit-note" data-id="${note.id}" type="button">编辑</button>
...
export const renderModules = () => {
  ...
  dom.moduleTitle.textContent = showFavoritesOnly ? "收藏夹" : getModuleName(selectedModuleId);
};

export const renderFavoriteToggle = () => {
  const state = getState();
  const bookId = activeBookId();
  const favoriteCount = bookId 
    ? state.libraryItems.filter((item) => item.bookId === bookId && item.isFavorite).length 
    : state.libraryItems.filter((item) => item.isFavorite).length;
  
  if (dom.libraryFavoriteToggle) {
    dom.libraryFavoriteToggle.innerHTML = `
      <label class="favorite-toggle">
        <input type="checkbox" data-action="toggle-favorites-only" ${showFavoritesOnly ? "checked" : ""} />
        <span class="favorite-toggle-icon">★</span>
        <span class="favorite-toggle-label">收藏夹</span>
        <span class="favorite-toggle-count">${favoriteCount}</span>
      </label>
    `;
  }
};
...
export const renderLibrary = () => {
  ...
  if (showFavoritesOnly) {
    items = state.libraryItems.filter((item) => item.isFavorite);
  } else {
    items = state.libraryItems.filter((item) => item.module === selectedModuleId);
  }
  ...
  if (!items.length) {
    const message = showFavoritesOnly 
      ? (query ? `收藏夹中未找到包含「${query}」的资料。` : "收藏夹为空。点击资料卡片上的 ★ 按钮收藏。")
      : (query ? `未找到包含「${query}」的资料。` : "这个模块还没有资料。可以从收件箱确认归档，也可以手动新增。");
        ...
        <article class="library-card">
          <div class="card-title-row">
            ...
            <div class="card-title-actions">
              <button class="icon-button favorite-btn ${item.isFavorite ? "active" : ""}" data-action="toggle-library-favorite" data-id="${item.id}" title="${item.isFavorite ? "取消收藏" : "收藏"}" type="button">
                ${item.isFavorite ? "★" : "☆"}
              </button>
              <span class="muted small">${formatDate(item.updatedAt)}</span>
            </div>
          </div>
          <p class="card-text">${renderRichText(item.content, query)}</p>
          <div class="tag-row">${(item.tags || []).map((tag) => `<span class="tag ${tag === "主角" ? "tag-protagonist" : ""}">${tag === "主角" ? "★ " : ""}${highlightSearchTerm(tag, query)}</span>`).join("")}</div>
          <div class="card-actions">
...
export const renderEditor = () => {
  ...
  if (activeChapter) {
    renderChapterHighlight(activeChapter.body || "");
  } else {
    dom.chapterBodyInput.innerHTML = "";
  }
  
  dom.chapterWordCount.textContent = `${wordCount(activeChapter?.body || "")} 字`;
};

export const renderChapterHighlight = (text, isFocusMode = false) => {
  const highlightedIds = getHighlightedReferences();
  const state = getState();
  const bookId = activeBookId();
  
  const highlightedItems = state.libraryItems.filter(
    item => item.bookId === bookId && highlightedIds.includes(item.id)
  );
  
  const terms = highlightedItems.map(item => item.title).filter(Boolean);
  ...
  if (!isFocusMode && dom.chapterBodyInput) {
    const selection = window.getSelection();
    const range = selection.rangeCount > 0 ? selection.getRangeAt(0) : null;
    
    dom.chapterBodyInput.innerHTML = result;
    
    if (range && dom.chapterBodyInput.contains(range.commonAncestorContainer)) {
      selection.removeAllRanges();
    }
  }
  
  if (isFocusMode && dom.focusChapterBodyInput) {
    const selection = window.getSelection();
    const range = selection.rangeCount > 0 ? selection.getRangeAt(0) : null;
    
    dom.focusChapterBodyInput.innerHTML = result;
    
    if (range && dom.focusChapterBodyInput.contains(range.commonAncestorContainer)) {
      selection.removeAllRanges();
    }
  }
};
...
export const renderReferences = () => {
  ...
  dom.referenceList.innerHTML = analyzeButton + filterBar + scored
    .map(({ item, score }) => {
      ...
      return `
        <article class="reference-card ${isReferenceHighlighted(item.id) ? 'highlighted' : ''}" data-action="toggle-reference-highlight" data-id="${item.id}">
          <div class="card-title-row">
            <strong>${escapeHtml(item.title)}</strong>
            <span class="status-pill">${scoreText}</span>
            <span class="highlight-indicator">${isReferenceHighlighted(item.id) ? '★' : ''}</span>
          </div>
          <p class="card-text">${renderRichText(item.content.slice(0, 130))}${item.content.length > 130 ? "..." : ""}</p>
...
export const renderSearchSuggestions = () => {
  ...
  let allItems = state.libraryItems;
  if (bookId) {
    allItems = allItems.filter(item => item.bookId === bookId);
  }
...
export const renderFocusEditor = () => {
  ...
  if (activeChapter) {
    ...
    dom.focusChapterBodyInput.innerHTML = "";
  }
