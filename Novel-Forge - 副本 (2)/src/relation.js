import { persist, activeBookId, getModules, getState, getRelationTypes, getRelationType, updateRelationType, addRelationType, isRelationTypeInUse, removeRelationType, removeUnusedRelationTypes } from './state.js';
...
export const renderRelationGraph = (itemId) => {
  ...
  dom.relationView.innerHTML = `
    ...
    <div class="relation-graph-container">
          ...
          return `
            <g class="relation-node-group" data-item-id="${node.id}" data-item-title="${escapeHtml(node.label)}" data-item-content="${escapeHtml(content)}" data-item-tags="${escapeHtml(tags)}" data-relation-type="${node.relationType || ''}" data-level="${node.level}" data-source-id="${node.sourceId || ''}">
              ...
              <circle 
                cx="${node.isCentral ? 0 : node.x}" 
                cy="${node.isCentral ? 0 : node.y}" 
                r="${node.isCentral ? 40 : node.level === 1 ? 30 : 22}" 
                fill="${node.color}"
                class="relation-node"
              />
              <text 
      ...
      <button id="addRelationButton" class="add-relation-button-float" type="button" title="添加关联">
        <span>+</span>
      </button>
    </div>
  `;
...
const setupRelationEventListeners = () => {
  ...
  if (addRelationButton) {
    addRelationButton.addEventListener('click', (e) => {
      e.stopPropagation();
      hideTooltip();
      showAddRelationModal(selectedItemId);
    });
  }
...
const showAddRelationModal = (fromId) => {
...
const showManageRelationTypesModal = () => {
  ...
  const modalHTML = `
    <dialog id="manageRelationTypesModal" class="modal">
      ...
      <div class="modal-body">
        ...
        ${relationTypes.length < 100 ? `
          <button type="button" id="addNewRelationTypeBtn" class="add-relation-type-btn">
            + 新增关系类型
          </button>
        ` : `
          <div class="relation-types-limit">已达到最大关系类型数量 (100)</div>
        `}
  ...
  const nameInputs = modal.querySelectorAll('.type-name-input');
  ...
  const colorPickers = modal.querySelectorAll('.type-color-picker');
  ...
  const deleteBtns = modal.querySelectorAll('.type-delete-btn');
